...
public class BasicTransformations {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            DataSet<Tuple8<Integer, String, String, String, Integer, Double, String, Double>> computedOrders = rawOrders.map(new MapComputeTotalOrderValue());
            ...
            DataSet<Tuple2<String, String>> customerTags = rawOrders.flatMap(new FlatMapExtractCustomerTags());

            Utils.printHeader("Customer and Tags extracted as separate dataset");
            customerTags.first(15).print();
            ...
            DataSet<Tuple2<Integer, Double>> totalOrders = orderColumns.aggregate(SUM, 0).and(SUM, 1); //Total Order Value

            //Extract the Summary row tuple, by converting the DataSet to a List and
            //fetching the first record
            Tuple2<Integer, Double> sumRow = (Tuple2<Integer, Double>) totalOrders.collect().get(0);

            Utils.printHeader("Aggregated Order Data ");
            System.out.println(" Total Order Value = " + sumRow.f1
                    + "  Average Order Items = " + sumRow.f0 * 1.0 / computedOrders.count());
            ...
            DataSet<Tuple3<String, Integer, Double>> productOrderSummary
                    ...
                    = computedOrders.map(i -> Tuple3.of(i.f2, 1, i.f7)) //Subset of columns
            ...
            productOrderSummary.map(new MapFunction<Tuple3<String, Integer, Double>,
